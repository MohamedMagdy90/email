...
join the automation queue?"

## Root cause — a boundary, not a bug

The lanes count `discovered_leads` and nothing else:
`countApprovableLeads()` → `SELECT count(*) FROM discovered_leads WHERE
status='pending' AND email <> ''`. A CSV import goes through
`POST /api/contacts/bulk` → `INSERT INTO contacts`, which never creates a lead
row. So the trigger counter physically could not move, and the 30k would have
sat at `status='new'` for ever.

Compounding it, the automation is an APPROVAL machine: it emails
`approve.contactIds` — only the contacts its own approve step just created — so
even a contact that somehow entered the queue would not have been emailed.

## The trap that made the obvious fix a no-op

Simply backfilling pool rows for the existing contacts does NOT work. Approve
does `INSERT ... ON CONFLICT (email) DO NOTHING RETURNING id`; on conflict it
returns nothing, the id never reaches `contactIds`, and the run bails with
"Every lead in that batch was already a contact — nothing to email." That is a
batch burned per cooldown, for ever, sending zero. **Both halves had to ship
together.**

## 1. New imports can go to the pool

- [x] `pool.ts` `importLeadsToPool()` — writes `discovered_leads`,
      `status='pending'`, `dedup_key='e:<email>'` (same shape discovery uses, so
      an import and a crawl cannot file the same person twice), `enriched=1`
      (we were handed the address — never spend a crawl on it),
      `source_id='__import__'`, audience tagged

- [x] Deliberately NOT reusing `insertDiscovered`: it drops any lead whose
      address is already a Contact — right when the crawler re-finds a company
      you have, exactly wrong when the operator is asking for these people to be
      queued — and it claims the domain for crawling, which an import never needs

- [x] Free-mail addresses carry no `domain`, or one gmail lead would block every
      other one through domain-level de-duplication

- [x] Import modal: a destination chooser (Review pool ▸ automation · Contacts
      only) defaulting to the pool, plus a Customer/Partner picker

## 2. The 30,000 already in production

- [x] `adoptContactsToPool()` — one-time backfill creating a pending pool row
      per never-emailed contact. Chunked 500 at a time behind a job, because
      30k single-row inserts inside a request is a timeout

- [x] `approveLeads()` now ADOPTS an existing contact when the sends ledger is
      empty for it and it is not unsubscribed/bounced. Ground truth is `sends`,
...
- [x] Idempotent: the predicate excludes contacts that already have a pool row
- [x] Contacts screen: a banner that appears only when there is something to do,
      with a live progress bar, and goes away for good afterwards

## 3. Don't corrupt the discovery health metric

- [x] `fillrate.ts` excludes `__import__` rows from both the inflow series and
      the "N companies arrived, none with an email" reason. That card answers
      "is DISCOVERY feeding the automation?" — a 30k CSV would have read as a
      colossal spike for one bucket and a total collapse for every bucket after,
      while saying nothing about whether the crawler still works

## Also fixed in passing

- `POST /api/contacts/bulk` never wrote `audience` at all — its INSERT column
  list omitted it — so every imported contact was untagged and unroutable to the
  partner lane. Now accepts it per-row or per-request.

## Verified

`scripts/verify-import-pool.ts` — **35/35** offline on a scratch DB: the bug
reproduced (30 contacts → lane reads 0) · 25 of 30 adoptable with the 3 emailed,
1 unsubscribed and 1 bounced correctly excluded · backfill queues 25 and writes
nothing to contacts · **second press scans 0** · approving a backfilled batch
returns `added:0, adopted:10, contactIds:10` — the line that used to be 0 ·
an already-emailed contact still refused · a genuinely new import still creates
a contact and lands in the right lane · `cleanEmail` repairs `//info@broken.com`
rather than discarding it · free-mail leads carry no domain · imports never
enter the crawl queue · the fill rate ignores imports **but still counts a real
crawled lead** (checked with both lanes live and the 15s cache forced, or the
assertion would have proved nothing).

Regressions: `verify-fillrate` 57/57 · `verify-recheck` 28/28 · `verify-names`
29/29 · `verify-schedule` 49/49. backend `tsc` clean · frontend `tsc` clean ·
`vite build` clean.

**Live over HTTP**, full chain on a scratch DB: 3 contacts imported the old way
→ both lanes read 0 (bug reproduced) → `/api/pool/adoptable` says 3 →
backfill job `{scanned:3, added:3}` → **lane reads customer:3** → second press
returns `adoptable:0` and HTTP 400 "No contacts are waiting to be queued" →
manual run → **`sent:3`**, note *"Approved 3 customer contact(s) from the pool
(3 already on file, never emailed)"*. The Resend guard refused the run first
time, exactly as designed.

⚠️ Note for the operator: the default daily ceiling is **300, shared across both
lanes**. 30,000 ÷ 300 ≈ 100 days. Raise it in Settings → Automation before this
matters. Nothing was deployed — this is local only.

⚠️ Incident: the apply model deleted `deleteContacts`,
`deleteContactsMatching` and `setContactsCategory` from `api.ts` while adding
the new methods, and invented a bogus `normalizeAudience` in `index.ts` that
...
The "Also sweep the whole country's web" option on a Web search source is gone.
It walked Common Crawl's index of a country's ccTLD, which answers "which HOSTS
exist under .qa" — not "which businesses are there" — so it kept filing parked
domains, portals, blogs and campaign sites as company leads, each named after
its own hostname, and buried the genuine keyword results underneath them. The
host/category filter added earlier cut the volume of that, never its character.

Killed at every level, so nothing can bring it back by accident:

- [x] `SWEEP_COUNTRY_ENABLED = false` in `discovery.ts`, checked BEFORE a row's
      own flag — a source still carrying `sweep_country=1` plans queries only.
      Typed `boolean`, not inferred `false`, so the machinery stays live code to
      the compiler and this is a genuine one-line reversal

- [x] The API never writes a 1: POST forces 0, PUT clears whatever was there —
      and clearing it resets the cursor, because dropping the index pages
      SHORTENS the plan and a stale cursor resumes at the wrong step

- [x] Boot migration clears the column on every existing source, unguarded this
      time: with no way to switch it on there is no "the user meant it" case to
      protect, unlike the earlier default-off migration

- [x] Checkbox removed from the Add/edit source modal; `sweepCountry` no longer
      sent or typed on the client. The "hundreds–thousands" promise came from
      the sweep, so that copy went with it

- [x] `verify-search` / `verify-batch` assert the retirement instead of the old
      behaviour, and both dropped their live Common Crawl calls — a retired path
      must not be able to fail a run because a free public index was resting.
      The token/host filter keeps its offline tests to guard the reversal
